...
PASSTHROUGH_LB_IP=$(kubectl get svc -n istio-system passthrough-ingressgateway -o=jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
echo "# Passthrough hostnames" >> /etc/hosts

for host in $PASSTHROUGH_HOSTS; do
    echo "${PASSTHROUGH_LB_IP} ${host}" >> /etc/hosts
done
