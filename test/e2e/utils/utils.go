...
func SetupTestPlatform(t *testing.T, platform *types.TestPlatform) { //nolint:funlen
	...
	teststructure.RunTestStage(t, "SETUP", func() {
		...
		output, err = platform.RunSSHCommandAsSudo(`cd ~/app/build && version=$(awk -F ' := ' '/PACKAGE_VERSION/ {print $2; exit}' ~/app/Makefile) && ./zarf package deploy "zarf-package-software-factory-amd64-\$version.tar.zst" --components optional-tools-linux-amd64 --confirm`)
		require.NoError(t, err, output)
