...
func TestAllServicesRunning(t *testing.T) { //nolint:funlen
	...
	teststructure.RunTestStage(platform.T, "TEST", func() {
		...
		output, err = platform.RunSSHCommandAsSudo(`kubectl wait statefulset/gitlab-minio-minio-instance-ss-0 -n gitlab-minio --for=condition=Ready --timeout=1200s`)
		require.NoError(t, err, output)
		...
		output, err = platform.RunSSHCommandAsSudo(`kubectl wait statefulset/velero-minio-minio-instance-ss-0 -n velero-minio --for=condition=Ready --timeout=1200s`)
		require.NoError(t, err, output)
