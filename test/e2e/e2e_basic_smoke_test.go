...
func TestAllServicesRunning(t *testing.T) { //nolint:funlen
	...
	teststructure.RunTestStage(platform.T, "TEST", func() {
		...
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan gitlab.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, jobKill))
		require.NoError(t, err, output)
		// Ensure that Jenkins does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan jenkins.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, jobKill))
		require.NoError(t, err, output)
		// Ensure that Jira does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan jira.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, jobKill))
		require.NoError(t, err, output)
		// Ensure that Confluence does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan confluence.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, jobKill))
		require.NoError(t, err, output)
		// Ensure that Artifactory does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan artifactory.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, jobKill))
		require.NoError(t, err, output)
