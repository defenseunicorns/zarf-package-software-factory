...
func TestAllServicesRunning(t *testing.T) { //nolint:funlen
	...
	teststructure.RunTestStage(platform.T, "TEST", func() {
		...
		output, err = platform.RunSSHCommandAsSudo(`cd ~/app/backup-and-restore/gitlab; ~/app/build/zarf p c --confirm --set BACKUP_FILENAME="$(kubectl exec -i -n gitlab -c toolbox $(kubectl get pod -n gitlab -l app=toolbox -o jsonpath="{.items[?(@.metadata.labels.pod-template-hash)].metadata.name}") -- s3cmd ls s3://gitlab-backups | awk "{split(\$NF,a,\"/\"); print a[length(a)]; exit}")" --set DELETE_REMOTE_BACKUP_FILE="no"`)
		require.NoError(t, err, output)
