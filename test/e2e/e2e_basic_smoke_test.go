...
func TestAllServicesRunning(t *testing.T) { //nolint:funlen
	...
	teststructure.RunTestStage(platform.T, "TEST", func() {
		...
		portforward := "kubectl port-forward -n istio-system $(kubectl get pods -n istio-system -o name | grep ingressgateway) 443:8443 & sleep 5 &&"
		killJobAndExitWithProperCode := "; EXITCODE=$?; kill %1; exit $EXITCODE"
		// Ensure that Jenkins is available outside of the cluster.
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s timeout 1200 bash -c "while ! curl -L -s --fail --show-error https://jenkins.bigbang.dev/login > /dev/null; do sleep 5; done" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		// Ensure that Confluence is available outside of the cluster.
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s timeout 1200 bash -c "while ! curl -L -s --fail --show-error https://confluence.bigbang.dev/status > /dev/null; do sleep 5; done" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		// Ensure that Jira is available outside of the cluster.
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s timeout 1200 bash -c "while ! curl -L -s --fail --show-error https://jira.bigbang.dev/status > /dev/null; do sleep 5; done" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		// Ensure that GitLab is available outside of the cluster.
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s timeout 1200 bash -c "while ! curl -L -s --fail --show-error https://gitlab.bigbang.dev/-/health > /dev/null; do sleep 5; done" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		...
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s timeout 1200 bash -c "while ! curl -L -s --fail --show-error https://artifactory.bigbang.dev/artifactory/api/system/ping > /dev/null; do sleep 5; done" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		...
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan gitlab.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		// Ensure that Jenkins does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan jenkins.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		// Ensure that Jira does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan jira.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		// Ensure that Confluence does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan confluence.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
		// Ensure that Artifactory does not accept TLSv1.1
		output, err = platform.RunSSHCommandAsSudo(fmt.Sprintf(`%s sslscan artifactory.bigbang.dev | grep "TLSv1.1" | grep "disabled" %s`, portforward, killJobAndExitWithProperCode))
		require.NoError(t, err, output)
